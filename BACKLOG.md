...
详见 commit log 与 `@cutie-crypto/connector-core` 0.2.0 changelog。
...
**2026-05-08 状态**：SRT smoke 期间未能自然复现。等下次复现一并抓 stderr → 修。这一项不阻塞日常使用——用户最多在 service 起来后第一个 task 看到一次 RUNNER_FAILURE，立即重试即成功，对 UX 是 nuisance 不是阻断。

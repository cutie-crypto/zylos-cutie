...
export type RouteBacktestResult =
  | { status: 'success'; response: BacktestResponse }
  | { status: 'error'; error_type: BacktestRouteErrorType | string; error_message: string };
 ...
 */
function mapBacktestRouteErrorType(routeErrorType: string): RunnerErrorEnvelope['error_type'] {
...
export class ZylosPlatformAdapter implements CorePlatformAdapter<ZylosAdapterConfig> {
  ...
  private connectorConfig: ZylosCutieConfig | null = null;
  ...
  private async handleBacktestTask(input: TaskInput): Promise<AgentResult | RunnerErrorEnvelope> {
    ...
    const runId = taskEnvelope['run_id'] as string | undefined;
  ...
  augmentHeartbeat(envelope: Record<string, unknown>): Record<string, unknown> {
    ...
    const reportableTools = tools.filter(
      (t) => t.health === 'ok' || t.health === 'unavailable',
    );
   ...
   */
  async routeBacktestTask(
    taskEnvelope: Record<string, unknown>,
  ): Promise<RouteBacktestResult> {
    const providerToolId = taskEnvelope['provider_tool_id'] as string | undefined;
    const tools = this.connectorConfig?.backtest_tools ?? [];

    let tool: BacktestToolConfig | undefined;

...
import { loadConfig, saveConfig, type BacktestProviderSource } from '../config.js';
...
/* ------------------------------------------------------------------ */
/*  Arg parsing (zero-dep, process.argv only)                          */
/* ------------------------------------------------------------------ */
...
async function cmdAdd(flags: Record<string, string | boolean>): Promise<void> {
  ...
  const source: BacktestProviderSource = {
    id,
    kind: 'external_http',
    base_url: base,
  ...
  if (catalogResult.ok) {
    ...
    for (const ct of catalogTools) {
      ...
      const tool: import('../config.js').BacktestToolConfig = {
        tool_id: ct.tool_id,
        kind: 'external_http',
        name: ct.name,
        ...
        health: healthResult.ok ? (ct.health ?? 'ok') : ('unavailable' as const),
        param_schema: ct.param_schema ?? null,
        expected_outputs: ct.expected_outputs ?? [],
        source_id: source.id,
        last_seen_at: Date.now(),
      };
      if (ct.data_source !== undefined) tool.data_source = ct.data_source;
...
async function cmdTest(sourceId: string): Promise<void> {
  const config = loadConfig();

 ...
 */
const POST_ANSWER_MARKERS = [
    /^hook:\s+/,
    /^\d{4}-\d{2}-\d{2}T[\d:.]+Z?\s+ERROR\s+/,
    /^tokens used\s*$/i,
    /^\d+\s*$/, // 单独数字行（token 计数）
];
const META_HEADER_MARKERS = [
    /^WARNING:\s+/,
...
export function extractCodexAnswer(stdout) {
    if (!stdout)
        return '';
...
function cleanupSegment(lines, drops) {
...
//# sourceMappingURL=codex-stdout-parser.js.map
...
const ALLOWED_WRAPPER_TYPES = new Set([
    'python_inprocess',
    'local_cli',
    'local_http',
]);
const CATALOG_TOOL_ALLOWED_KEYS = new Set([
    'tool_id',
    'kind',
    'name',
    'description',
...
export class BacktestContractError extends Error {
    error_type = 'PROVIDER_CONTRACT_VIOLATION';
}
/* ------------------------------------------------------------------ */
...
export function isSensitiveKey(key) {
    ...
    if (SENSITIVE_KEY_NAMES.has(normalized))
        return true;
    return /(_api_key|_secret|_token|_password|_passwd|_pwd|_private_key|_credential|_bearer|_access_key)$/.test(normalized);
}
export function looksLikeHighEntropyToken(value) {
    if (/\s/.test(value))
        return false;
    const dottedWords = value.split('.');
    const looksDottedIdentifier = dottedWords.length >= 2 && dottedWords.every(seg => /^[A-Za-z0-9_-]+$/.test(seg) && seg.length <= 40);
...
/* ------------------------------------------------------------------ */
export function scrubSnapshotValue(value) {
    if (Array.isArray(value)) {
        return value.map(item => scrubSnapshotValue(item));
    }
...
/* ------------------------------------------------------------------ */
function querySearchLooksSensitive(params) {
...
export function scrubReportUrl(reportUrl) {
...
/* ------------------------------------------------------------------ */
export function validateProviderCatalogTool(tool) {
    ...
    for (const key of Object.keys(tool)) {
        if (CATALOG_TOOL_ALLOWED_KEYS.has(key))
            continue;
        if (key.startsWith('x_') || key.startsWith('x-'))
            continue;
...
/* ------------------------------------------------------------------ */
function validateParamField(key, value, def) {
    const type = typeof def.type === 'string' ? def.type : undefined;
    if (type) {
        switch (type) {
            case 'string':
                if (typeof value !== 'string')
                    return `provider_params field "${key}" must be a string`;
                break;
            case 'boolean':
                if (typeof value !== 'boolean')
                    return `provider_params field "${key}" must be a boolean`;
                break;
            case 'integer':
...
export function validateProviderParams(params, schema) {
    ...
    for (const key of required) {
        const propDef = isJsonObject(props[key]) ? props[key] : undefined;
...
/* ------------------------------------------------------------------ */
function assertNoFloatMoneyFieldsDeep(value, context) {
    if (Array.isArray(value)) {
    ...
    for (const [key, child] of Object.entries(value)) {
        if (MONEY_FIELD_NAMES.has(key.toLowerCase()) && typeof child === 'number') {
            throw new BacktestContractError(`Provider response ${context} field "${key}" must be a decimal string, not a JSON number (money/quantity precision rule)`);
        }
        if (typeof child === 'number' && !Number.isFinite(child)) {
            throw new BacktestContractError(`Provider response ${context} field "${key}" must not be NaN/Infinity`);
        }
...
export function assertNoFloatMoneyFields(rows, context) {
    for (const row of rows) {
        assertNoFloatMoneyFieldsDeep(row, context);
    }
}
//# sourceMappingURL=backtest-safety.js.map
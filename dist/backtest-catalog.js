 ...
 */
function catalogToolToConfig(tool, source) {
    const cfg = {
        tool_id: tool.tool_id,
        kind: tool.kind ?? 'external_http',
        name: tool.name,
    ...
    if (tool.data_source !== undefined)
        cfg.data_source = tool.data_source;
    return cfg;
}
/**
 ...
 */
export async function refreshAllSources(config) {
    ...
    const disabledSourceIds = new Set((config.backtest_provider_sources ?? []).filter((s) => !s.enabled).map((s) => s.id));
    ...
    const retainedTools = existingTools.filter((t) => !refreshingSourceIds.has(t.source_id) && !disabledSourceIds.has(t.source_id));
    ...
    const toolIdOwner = new Map(); // tool_id -> source_id
...
export function resetFailureCounts() {
    sourceFailureCounts.clear();
}
//# sourceMappingURL=backtest-catalog.js.map